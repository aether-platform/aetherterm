...
async def test_session_restoration():
    ...
    await sio1.emit(
        "create_terminal",
        {"session": "test_session_1", "cols": 80, "rows": 24, "user": "", "path": ""},
    )
    ...
    await sio1.emit(
        "terminal_input",
        {"session": "test_session_1", "data": 'echo "Hello from first connection"\n'},
    )

    await asyncio.sleep(1)

    await sio1.emit("terminal_input", {"session": "test_session_1", "data": "date\n"})
