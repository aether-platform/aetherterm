...
class TerminalHandler:
    ...
    async def handle_create_terminal(self, sid: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Use the requested session ID directly for simplified global workspace
            ...
            if existing_session:
                ...
                if buffer_content:
                    await self.sio.emit(
                        "terminal_output", {"session": session_id, "data": buffer_content}, room=sid
                    )

                await self.sio.emit(
                    "terminal_ready", {"session": session_id, "status": "resumed"}, room=sid
                )
                return

            # Create new session
            session = await self.terminal_service.create_session(
                session_id=session_id, user_name=user_name, path=path, rows=rows, cols=cols
            )
            ...
            self.terminal_service.set_output_callback(
                session_id, lambda sid, data: self._handle_output(sid, session_id, data)
            )

            self.terminal_service.set_close_callback(
                session_id, lambda sid: self._handle_close(sid, session_id)
            )

            # Notify client
            await self.sio.emit(
                "terminal_ready", {"session": session_id, "status": "created"}, room=sid
            )

        except Exception as e:
            log.error(f"Error creating terminal: {e}")
            await self.sio.emit("terminal_error", {"error": str(e)}, room=sid)
    ...
    async def handle_terminal_input(self, sid: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not success:
                await self.sio.emit(
                    "terminal_error", {"error": "Session not found or closed"}, room=sid
                )
    ...
    async def _handle_output(self, sid: str, session_id: str, data: str) -> None:
        ...
        for client_sid in session.client_sids:
            await self.sio.emit(
                "terminal_output", {"session": session_id, "data": data}, room=client_sid
            )
    ...
    async def _handle_close(self, sid: str, session_id: str) -> None:
        ...
        # Notify all clients
        for client_sid in session.client_sids:
            await self.sio.emit("terminal_closed", {"session": session_id}, room=client_sid)
