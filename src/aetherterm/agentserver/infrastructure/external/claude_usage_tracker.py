...
class ClaudeUsageTracker:
    ...
    def _calculate_cost_row(self, row) -> float:
        ...
        input_cost = ((row['input_tokens'] + row['cache_creation_tokens']) * pricing["input"]) / 1_000_000
    ...
    def get_daily_breakdown(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        daily_stats.columns = ['date', 'cost', 'requests', 'input_tokens', 'output_tokens', 
    ...
    def get_session_blocks(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        block_stats = df_filtered.groupby('block').agg({
            'cost': 'sum',
            'message_id': 'count',
            'timestamp': ['min', 'max']
        }).reset_index()
    ...
    def get_model_breakdown(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        for _, row in model_stats.iterrows():
            ...
            result.append({
                ...
                "requests": int(row['message_id']),
                "tokens": int(tokens),
                "percentage": round(float(row['cost'] / total_cost * 100), 1) if total_cost > 0 else 0
            })
