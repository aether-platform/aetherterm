        ...
        try:
            ...
            if result and result[0] is not None:
                return {
                    ...
                    "period_days": days,
                    "available": True,
                    "average_cost_per_request": round((result[4] or 0.0) / max(result[3] or 1, 1), 4),
                    "cost_per_1k_tokens": round((result[4] or 0.0) / max((result[2] or 1) / 1000, 0.001), 4)
                }
            else:
                return {
                    "total_cost": 0.0,
...
class AIService:
        ...
        if self.provider == "lmstudio":
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(f"{self.lmstudio_url}/v1/models", timeout=aiohttp.ClientTimeout(total=5)) as response:
    ...
    def get_cost_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            log.info(f"Claude stats retrieved: available={claude_stats.get('available')}, requests={claude_stats.get('requests')}")
        ...
        if self.provider == "claude":
            ...
            estimated_cost = (
                (self.session_usage["input_tokens"] / 1000) * rates["input"] +
                (self.session_usage["output_tokens"] / 1000) * rates["output"]
            )
